...
//
// The fitting-room check never lets a try-on reach Gemini: it refuses to run
// unless the server was started with TRYON_MOCK=1. Exit code 1 on failure.
...
try {
  ...
  console.error(`Nothing is answering at ${BASE}. Start the site first (npm run build && npm run start:mock).`);
  process.exit(1);
}
